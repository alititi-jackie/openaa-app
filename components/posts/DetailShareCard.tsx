...
export function DetailShareCard({ path, title, text, className, hint = "一键调用系统分享，不支持时会自动复制链接", onClick }: DetailShareCardProps) {
  const cardClassName = className ? `${defaultClassName} ${className}` : defaultClassName;
  const label = (
