...
| posts management | Complete | New `/admin/posts` handles jobs/housing/marketplace/services in one unified table. Old `secondhand` maps to new `marketplace`. |
| services admin page | Covered by posts | Old `/admin/services` is represented by `/admin/posts?type=services` in the new architecture. |
